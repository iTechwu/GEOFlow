...
if ! printf '%s' "${APP_KEY:-}" | grep -Eq '^base64:[A-Za-z0-9+/]{43}=$'; then
  echo "[entrypoint-prod] error: APP_KEY must be provided before the production container starts" >&2
  exit 1
fi
...
if [ "${AUTO_WAIT_FOR_DB:-true}" = "true" ] && [ "${DB_CONNECTION:-}" = "pgsql" ]; then
  DB_HOST_VALUE="${DB_HOST:-dofe-postgres}"
  ...
  until pg_isready -h "${DB_HOST_VALUE}" -p "${DB_PORT_VALUE}" -U "${DB_USER_VALUE}" -d "${DB_NAME_VALUE}" >/dev/null 2>&1; do
    ...
    if [ "$attempt" -ge "$DB_WAIT_ATTEMPTS" ]; then
      echo "[entrypoint-prod] error: PostgreSQL not reachable at ${DB_HOST_VALUE}:${DB_PORT_VALUE} after ${attempt} attempts" >&2
      exit 1
    fi
