...
Настройте учётные данные клиента SSO и redirect URI в `.env`. Установщик не создаёт локальных администраторов или паролей; первый вход через SSO создаёт локальную проекцию, связанную с subject SSO.

`geoflow:install` инициализирует схему и необязательные демо-данные. API принимают только bearer tokens SSO.
