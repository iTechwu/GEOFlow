...
final class McpController extends Controller
{
    ...
    private function tools(): array
    {
        ...
        $taskConfiguration = [
            'name' => ['type' => 'string'],
            'title_library_id' => ['type' => 'integer', 'minimum' => 1],
            'image_library_id' => $nullableInteger,
            'prompt_id' => ['type' => 'integer', 'minimum' => 1],
            'ai_model_id' => ['type' => 'integer', 'minimum' => 1],
            'author_id' => $nullableInteger,
            'knowledge_base_id' => $nullableInteger,
            'knowledge_base_ids' => ['type' => 'array', 'items' => ['type' => 'integer', 'minimum' => 1]],
            'fixed_category_id' => $nullableInteger,
            ...
            'draft_limit' => ['type' => 'integer', 'minimum' => 1],
            'category_mode' => ['type' => 'string', 'enum' => ['smart', 'fixed']],
            'model_selection_mode' => ['type' => 'string', 'enum' => ['fixed', 'smart_failover']],
            'publish_scope' => ['type' => 'string', 'enum' => ['local_and_distribution', 'distribution_only', 'local_only']],
            'distribution_strategy' => ['type' => 'string', 'enum' => ['broadcast', 'round_robin', 'random_balanced']],
            'sso_team_id' => ['type' => 'string'],
        ];
        ...
        return [
            ...
            ['name' => 'geoflow.articles.create', 'description' => 'Create an article with draft, review, risk, and workflow fields after GEO content generation.', 'inputSchema' => ['type' => 'object', 'properties' => array_merge(['title' => ['type' => 'string'], 'content' => ['type' => 'string'], 'excerpt' => ['type' => 'string'], 'keywords' => ['type' => 'string'], 'meta_description' => ['type' => 'string'], 'category_id' => ['type' => 'integer', 'minimum' => 1], 'author_id' => ['type' => 'integer', 'minimum' => 1], 'task_id' => $nullableInteger, 'status' => ['type' => 'string', 'enum' => ['draft', 'published', 'private']], 'review_status' => ['type' => 'string', 'enum' => ['pending', 'approved', 'rejected', 'auto_approved']], 'risk_override_reason' => ['type' => 'string'], 'is_ai_generated' => ['type' => 'boolean']], $idempotency), 'required' => ['title', 'content', 'category_id', 'author_id'], 'additionalProperties' => false]],
            ...
            ['name' => 'geoflow.tasks.update', 'description' => 'Update a tenant-scoped task configuration, including scheduling, category, publication, and distribution options.', 'inputSchema' => ['type' => 'object', 'properties' => array_merge(['task_id' => ['type' => 'integer', 'minimum' => 1]], $taskConfiguration, $idempotency), 'required' => ['task_id'], 'additionalProperties' => false]],
            ...
            ['name' => 'geoflow.jobs.cancel', 'description' => 'Cancel one tenant-scoped pending or running execution record. Completed jobs cannot be cancelled.', 'inputSchema' => ['type' => 'object', 'properties' => array_merge(['job_id' => ['type' => 'integer', 'minimum' => 1], 'reason' => ['type' => 'string']], $idempotency), 'required' => ['job_id'], 'additionalProperties' => false]],
            ...
            ['name' => 'geoflow.url_import.run', 'description' => 'Queue tenant-scoped URL import preview processing. Returns immediately; poll geoflow.url_import.status for completed or failed.', 'inputSchema' => ['type' => 'object', 'properties' => array_merge(['job_id' => ['type' => 'integer', 'minimum' => 1]], $idempotency), 'required' => ['job_id'], 'additionalProperties' => false]],
            ...
            ['name' => 'geoflow.analytics.goals', 'description' => 'Read tenant-scoped content production goals and attainment for one month (YYYY-MM). Goals are managed in the admin console; months without goals return an empty list.', 'inputSchema' => ['type' => 'object', 'properties' => ['month' => ['type' => 'string', 'pattern' => '^\\d{4}-(0[1-9]|1[0-2])$']], 'additionalProperties' => false]],
            ['name' => 'geoflow.enterprise_knowledge.list', 'description' => 'List bounded tenant-scoped enterprise knowledge project metadata without draft content.', 'inputSchema' => ['type' => 'object', 'properties' => ['search' => ['type' => 'string', 'maxLength' => 120], 'status' => ['type' => 'string', 'enum' => ['draft', 'queued', 'processing', 'reviewing', 'published', 'failed']], 'limit' => ['type' => 'integer', 'minimum' => 1, 'maximum' => 100]], 'additionalProperties' => false]],
            ...
            ['name' => 'geoflow.enterprise_knowledge.delete', 'description' => 'Delete a tenant-scoped enterprise knowledge project and its unreferenced published knowledge base. Requires confirmation=DELETE.', 'inputSchema' => ['type' => 'object', 'properties' => array_merge(['project_id' => ['type' => 'integer', 'minimum' => 1], 'confirmation' => ['type' => 'string', 'enum' => ['DELETE']]], $idempotency), 'required' => ['project_id', 'confirmation'], 'additionalProperties' => false]],
    ...
    private function callTool(Request $request, mixed $id, array $params, CatalogGeoFlowService $catalog, TaskLifecycleService $tasks, ArticleGeoFlowService $articles, MaterialLibraryService $materials, McpUrlImportService $urlImports, McpCapabilityService $capabilities, McpAnalyticsService $analytics, McpEnterpriseKnowledgeService $enterpriseKnowledge, McpSiteService $site, McpDistributionService $distribution, McpLeadService $leads, McpFrontendService $frontend, McpSystemService $system, McpToolInputValidator $inputValidator): JsonResponse
    {
        ...
        $data = match ($name) {
            ...
            'geoflow.analytics.goals' => $this->runReadTool($request, $name, $arguments, fn (array $args) => $analytics->goals($args, $auth)),
            'geoflow.enterprise_knowledge.list' => $this->runReadTool($request, $name, $arguments, fn (array $args) => $enterpriseKnowledge->list($args, $auth)),
            ...
            'geoflow.enterprise_knowledge.delete' => $this->runWriteTool($request, $name, $arguments, fn (array $args) => $enterpriseKnowledge->delete((int) $args['project_id'], (string) $args['confirmation'], $auth)),
            ...
            'geoflow.system.status' => $this->runReadTool($request, $name, $arguments, fn (array $args) => $system->status($auth)),
    ...
    {
        $required = match (true) {
            in_array($tool, ['geoflow.enterprise_knowledge.list', 'geoflow.enterprise_knowledge.status'], true) => 'materials:read',
